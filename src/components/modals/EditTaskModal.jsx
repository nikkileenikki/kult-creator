...
export default function EditTaskModal() {
  ...
  async function onSubmit(data) {
    ...
    if (willComplete) {
      showToast(`Task completed — ${data.coins} coins awarded to ${task.creatorName}`)
    } else {
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-white/[0.07]">
              <div>
                ...
                <Dialog.Description className="text-[12px] text-white/30 mt-0.5">
                  {task?.creatorName} · {task?.platform}
                </Dialog.Description>
              </div>
                ...
                {willComplete && (
                  <div className="flex items-center gap-2 px-3 py-2.5 bg-emerald-500/8 border border-emerald-500/20 rounded-lg">
                    ...
                    <span className="text-[12px] text-white/40">
                      Marking as complete will award <span className="text-emerald-300 font-semibold">{watch('coins')} coins</span> to {task?.creatorName}
                    </span>
                  </div>
                )}
              </div>
